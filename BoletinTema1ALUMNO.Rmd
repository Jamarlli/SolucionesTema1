...
author: "Javier Martínez Llinares"
...
```{r}
commonAnimals <- intersect(Animals2, mammals)
...
setdiff(Animals2, mammals) 

```
Los animales que no estan en ambos conjuntos son el Diplodocus, el Triceratops y el Branchiosaurus, que son tres dinosaurios

e.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos.

```{r}

# La diferencia entre ambos conjuntos es que Animals2 contiene algunos 
# dinosaurios mientras que mammals tan solo contiene mamiferos
...
#Presentan un comportamiento similar, y podriamos decir que ambas siguen la distribuvion Gaussiana, porque sus puntos se encuentran entre las lineas discontinuas
...
# b)
head(libReturn)
libReturn$results[1:5]

# c)
dim(libReturn$results)[1] # Tengo instaladas este número de librerias
...
# Las tres primeras estapas son averiguar en número de registros, el número de variables y la clase de las variables

dim(cabbages)[1] # Número de registros
dim(cabbages)[2] # Número de variables
colnames(cabbages) # Nombre de las variables

# b)
summary(is.na(cabbages)) # No hay ningún NA en ninguna de las variables de cabbages

# c)
library(car)
data("Chile")
dim(Chile)[1] # Número de registros
dim(Chile)[2] # Número de variables
colnames(Chile) # Nombre de las variables
summary(is.na(Chile)) # Dentro del conjunto Chile, hay varios NA repartidos en 
# varias variables: 1 en "sex", 11 en "education", 98 en "income", 17 en 
# "statusquo" y 168 en "vote". 
...
# Con los graficos se demuestra que las variables "HeadWt" y "VitC" verifican la 
# condicion de que los datos siguen una distribución Gaussiana
...
library(MASS)
data(cabbages)
hist(cabbages$VitC[which(cabbages$Cult == "c39")])
hist(cabbages$VitC[which(cabbages$Cult == "c52")])
# Comprobamos con los graficos que hay una diferencia significativa en los 
# valores de la vitamina C dependiendo de la variedad de calabaza, ya que las 
# c52 acostumbran a tener un rango de valores mayor que el rango de valores 
# de la vitamina C de las calabazas c39
